---
name: ferris-windows-dev
description: Windows platform rules and Win32/COM correctness for any code that runs on Windows, in any language. Use when writing, modifying, or reviewing code or scripts that run on Windows — even with no Win32 call in sight — for MAX_PATH and \\?\ long paths, case-insensitive filenames and reserved device names, files locked by a running process (sharing violations, linker LNK1168), DLL search order, console code pages and UTF-8 output, symlinks needing Developer Mode, or UAC elevation; and when calling Win32, COM, or NT APIs — windows.h, the Rust windows or windows-sys crates or raw extern system FFI, C# P/Invoke or DllImport, PowerShell touching the registry, services, or shell. Also triggers on HANDLE, HWND, HRESULT, GetLastError, CreateFileW, RegOpenKeyExW, CoInitializeEx, WinMain, message loops, UTF-16, any W- or A-suffixed API, and DLL boundary design. C++ and Rust naming or style belong to ferris-cpp and ferris-rust; LF/CRLF churn belongs to ferris-line-endings.
---

# Windows Platform Discipline

Invoke for ANY code that runs on Windows. The platform rules below apply with no API call in sight; the API rules apply to `*W`/`*A` functions from kernel32/user32/advapi32/ole32/shell32 and friends, COM interfaces, handles (`HANDLE`, `HWND`, `HDC`), registry, services, or shell — in any language, however trivial the snippet.

## Platform rules (no API call required)
