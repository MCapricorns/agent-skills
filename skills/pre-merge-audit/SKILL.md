...
2. **Deep cleanup (explicit intent only)** — proven cuts anywhere in scope per `references/cleanup.md`.
...
Only on explicit user intent; never escalate into edits on your own. Survey the scope first, then apply every safe, proven, in-scope cut end to end per `references/cleanup.md`, without per-item approval, then re-verify the cleanup diff.
